...
from fields_v4 import (
    F_物料编号, F_行业领域, F_物料内容, F_物料附件, F_提交人, F_紧急程度,
    F_美妆_投放平台, F_游戏_投放平台, F_保健食品_投放平台,
    F_预审_风险等级, F_预审_命中要点, F_预审_修改建议,
    F_法务_AI意见评价, F_法务_物料裁决, F_法务_最终修改意见,
)
...
def build_initial_review_card(
    record_id: str, fields: dict, *, image_key: Optional[str] = None,
    ...
    preview_elements = (
        [
            {"tag": "div", "text": {"tag": "lark_md", "content": "**物料预览**"}},
            {
                ...
                "alt": {"tag": "plain_text", "content": "物料图片"}, "mode": "crop_center",
            },
        ]
        if image_key
        else [{
    ...
    return {
        ...
        "elements": [
            ...
            {"tag": "div", "text": {"tag": "lark_md", "content": f"**投放平台**\n{_md(platform)}"}},
            *preview_elements,
            {"tag": "hr"},
            {"tag": "note", "elements": [{"tag": "plain_text", "content": "请确认内容无误后，选择是否启用 AI 审核"}]},
            {"tag": "action", "actions": [
                {
                    "tag": "button", "text": {"tag": "plain_text", "content": "🚀 开启 AI 审核"},
                    "type": "primary", "value": _action_value("start_ai_review", record_id, round_number),
                },
                {
                    "tag": "button", "text": {"tag": "plain_text", "content": "✏️ 修改物料"},
                    "type": "default", "url": detail_url,
                },
                {
                    "tag": "button", "text": {"tag": "plain_text", "content": "跳过 → 直发法务"},
                    "type": "default", "value": _action_value("skip_review", record_id, round_number),
                },
            ]},
        ],
    }
...
def build_operator_result_card(record_id: str, fields: dict, *, round_number: Optional[int] = None) -> dict:
    ...
    return {
        ...
        "elements": [
            ...
            {"tag": "hr"},
            {"tag": "note", "elements": [{"tag": "plain_text", "content": "请根据以上建议修改物料，修改完成后点击「重新提交」"}]},
            {"tag": "action", "actions": [
                {"tag": "button", "text": {"tag": "plain_text", "content": "🔍 查看AI审核意见"}, "type": "default", "url": kanban_url},
                {"tag": "button", "text": {"tag": "plain_text", "content": "✏️ 去修改物料"}, "type": "default", "url": detail_url},
                {"tag": "button", "text": {"tag": "plain_text", "content": "✅ 修改完成，重新提交"}, "type": "primary", "value": _action_value("resubmit", record_id, round_number)},
                {"tag": "button", "text": {"tag": "plain_text", "content": "⚖️ 转交法务复核"}, "type": "default", "value": _action_value("escalate_to_legal", record_id, round_number)},
            ]},
        ],
    }


def build_more_info_card(record_id: str, fields: dict, *, round_number: Optional[int] = None) -> dict:
    detail_url, kanban_url = _urls(record_id)
    points = _short(fields.get(F_预审_命中要点), 100) or "请补充物料相关信息"
    suggestion = _text(fields.get(F_预审_修改建议))
    elements = [
        {"tag": "div", "text": {"tag": "lark_md", "content": f"**需要补充**\n{_md(points)}"}},
    ]
    if suggestion:
        elements.append({"tag": "div", "text": {"tag": "lark_md", "content": f"**补充建议**\n{_md(suggestion)}"}})
    elements.extend([
        {"tag": "hr"},
        {"tag": "action", "actions": [
            {"tag": "button", "text": {"tag": "plain_text", "content": "✏️ 去补充"}, "type": "primary", "url": detail_url},
            {"tag": "button", "text": {"tag": "plain_text", "content": "🔍 查看AI审核意见"}, "type": "default", "url": kanban_url},
            {"tag": "button", "text": {"tag": "plain_text", "content": "✅ 补充完成，重新提交"}, "type": "default", "value": _action_value("resubmit", record_id, round_number)},
        ]},
    ])
    return {
        "config": {"wide_screen_mode": True},
        "header": {"title": {"tag": "plain_text", "content": "📝 请补充物料信息"}, "template": "orange"},
        "elements": elements,
    }


def build_legal_review_card(record_id: str, fields: dict, *, workbench_url: str) -> dict:
    risk = _text(fields.get(F_预审_风险等级)) or "未知"
    points = _short(fields.get(F_预审_命中要点), 80) or "（无）"
    content = _text(fields.get(F_物料内容))
    preview = content[:50] + ("…" if len(content) > 50 else "") if content else f"#{record_id[-8:]}"
    return {
        "config": {"wide_screen_mode": True},
        "header": {"title": {"tag": "plain_text", "content": "📨 新物料待法务复核"}, "template": "orange"},
        "elements": [
            {"tag": "div", "fields": [
...
def build_operator_transferred_card(record_id: str, fields: dict) -> dict:
    ...
    return {
        "config": {"wide_screen_mode": True},
        "header": {"title": {"tag": "plain_text", "content": "⏳ 物料已流转至法务，等待裁决"}, "template": "blue"},
        "elements": [
            {"tag": "div", "fields": [
            ...
            {"tag": "div", "text": {"tag": "lark_md", "content": f"**AI命中要点**\n{_md(points)}"}},
            {"tag": "hr"},
            {"tag": "note", "elements": [{"tag": "plain_text", "content": "审核已完成并转交法务团队复核。法务裁决后你将收到通知。"}]},
            {"tag": "action", "actions": [
                {"tag": "button", "text": {"tag": "plain_text", "content": "🔍 查看AI审核意见"}, "type": "primary", "url": kanban_url},
                {"tag": "button", "text": {"tag": "plain_text", "content": "📋 查看物料详情"}, "type": "default", "url": detail_url},
            ]},
        ],
    }


def build_verdict_card(record_id: str, fields: dict, *, round_number: Optional[int] = None) -> dict:
    ...
    passed = verdict == "通过"
    detail_url, kanban_url = _urls(record_id)
    elements = [{"tag": "div", "text": {"tag": "lark_md", "content": f"**物料内容**\n{_md(preview)}"}}]
    if not passed and final_suggestion:
        elements.append({"tag": "div", "text": {"tag": "lark_md", "content": f"**修改意见**\n{_md(final_suggestion)}"}})
    elif passed:
        elements.append({"tag": "div", "text": {"tag": "lark_md", "content": "物料已经过法务审核，可正常发布。"}})
    ...
    return {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": f"{'✅ 物料已通过法务审核' if passed else '📝 物料需修改后重新提交'} #{serial}"},
...
def build_final_failure_card(record_id: str, *, round_number: Optional[int] = None) -> dict:
    return {
        "config": {"wide_screen_mode": True},
        "header": {"title": {"tag": "plain_text", "content": FINAL_FAILURE_TITLE}, "template": "orange"},
        "elements": [
            {"tag": "div", "text": {"tag": "plain_text", "content": FINAL_FAILURE_BODY}},
            {"tag": "action", "actions": [{
                "tag": "button", "text": {"tag": "plain_text", "content": FINAL_FAILURE_BUTTON},
                "type": "primary", "value": _action_value("start_ai_review", record_id, round_number),
            }]},
        ],
    }
...
def build_card(card_type: str, record_id: str, fields: dict, payload: Optional[dict] = None) -> dict:
    payload = payload or {}
    round_number = payload.get("round")
    if card_type == "initial_review":
        return build_initial_review_card(
            record_id, fields, image_key=payload.get("image_key"), round_number=round_number,
        )
    ...
    if card_type == "legal_review":
        return build_legal_review_card(record_id, fields, workbench_url=_workbench_url())
