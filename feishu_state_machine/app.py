...
from review_service import review_version, validate_review_payload
from user_messages import (
    ACTION_ACCEPTED, ACTION_RETRY, REVIEW_SUBMIT_UNAVAILABLE, STALE_CARD,
    WORKBENCH_UNAVAILABLE, LATEST_REVIEW_AVAILABLE,
)
app = Flask(__name__)
...
def _register_timeline_feature() -> None:
    try:
        ...
        logger.warning(
            "event=timeline_routes_registration_failed error_category=optional_feature"
        )
...
def submit_review(record_id):
    ...
    allowed = {
        ...
        "review_intent", "expected_review_version",
    }
    ...
    if job and job["status"] == SUCCEEDED:
        ...
        if result.get("stale") is True:
            return jsonify({
                ...
                "message": LATEST_REVIEW_AVAILABLE,
            }), 409
...
def normalize_record(record_id, fields):
    return {
        ...
        "review_version": review_version(fields),
    }
...
def _register_ops_feature() -> None:
    try:
        ...
        logger.warning(
            "event=ops_routes_registration_failed error_category=optional_feature"
        )
...
def list_corrections():
    try:
        from memory_center import public_corrections

        return jsonify(public_corrections(preference_memory.list_all()))
    except Exception:
...
def _register_memory_feature() -> None:
    try:
        ...
        logger.warning(
            "event=memory_routes_registration_failed error_category=optional_feature"
        )
