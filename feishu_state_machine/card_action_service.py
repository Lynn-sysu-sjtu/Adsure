...
def _record_action_event(
    ...
    try:
        ...
        record_event(
            event_type,
            record_id=record_id,
            event_key=f"{event_type.replace('_', '-')}:job:{job_id}:round:{round_number}",
            summary={"round": round_number, **summary},
            actor_role="operator",
            job_id=job_id,
            store=store,
        )
        ...
        logger.exception(
            "event=audit_event_write_failed stage=card_action_boundary %s",
            context_fields(record_id=record_id, job_id=job_id),
        )
