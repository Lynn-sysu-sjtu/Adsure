...
新增能力使用四个独立的部署开关，全部默认关闭：

- `ADSURE_AUDIT_TIMELINE_ENABLED`
- `ADSURE_LEGAL_CARD_SYNC_ENABLED`
- `ADSURE_OPS_ENABLED`
- `ADSURE_MEMORY_CENTER_AVAILABLE`

缺少这些配置时等同旧版本行为。首次启动会以追加方式创建可选功能所需的表，并为投递表补充带默认值的列；不会重建表或批量改写旧任务。

开启 `ADSURE_AUDIT_TIMELINE_ENABLED` 后，法务工作台会在物料详情中异步显示已经真实发生的审核节点。旧记录不会根据当前状态推测补写历史；时间线读取或写入失败也不会阻断原审核、裁决和卡片投递。

开启 `ADSURE_LEGAL_CARD_SYNC_ENABLED` 后，一次法务裁决会为同一物料、同一轮次已经送达的法务卡创建独立的持久更新投递。关闭该开关不会影响裁决或运营通知。完整回滚前应先关闭开关，并等待在途的“法务卡片完成更新”投递结束；历史投递记录无需删除。

投递任务中心只有在 `ADSURE_OPS_ENABLED=True` 且维护账号、密码均已配置时才开放 `/ops/deliveries`；否则页面和接口均返回 404。该入口不出现在普通工作台中，部署时必须置于 HTTPS 和受限维护网络之后。手动重新发送只会重置同一条终止失败且尚无消息编号的投递，继续复用原幂等键和稳定发送 UUID。

开启 `ADSURE_MEMORY_CENTER_AVAILABLE` 后，“规则沉淀库”会显示合规记忆开关；SQLite 中的 `memory_center_enabled` 才决定审核时是否读取历史纠正，默认为 OFF。OFF 时不检索、不注入且不增加模型调用，但新的法务纠正仍按原逻辑沉淀；纠正读取或可选修正失败时会直接使用原审核结果。
