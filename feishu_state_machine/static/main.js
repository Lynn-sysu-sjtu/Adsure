...
function renderAiReviewCard(record) {
    ...
    <div class="detail-card">
        ...
        <div style="margin-bottom:10px;">
            ...
            <div class="review-full-text" style="background:#fffbe6;border:1px solid #ffe58f;border-radius:5px;padding:12px 14px;font-size:13px;color:#333;line-height:1.8;">${escapeHtml(hitPoints)}</div>
        </div>` : ""}
        ...
        <div style="margin-bottom:10px;">
            ...
            <div class="review-full-text" style="background:#fff1f0;border:1px solid #ffccc7;border-radius:5px;padding:12px 14px;font-size:13px;color:#cf1322;line-height:1.8;">${escapeHtml(suggestion)}</div>
        </div>` : ""}
        ...
        <div style="margin-bottom:10px;">
            ...
            <div class="review-full-text" style="font-size:12px;color:#555;padding:4px 0;">${escapeHtml(entities)}</div>
        </div>` : ""}
...
function showDetail(record) {
    ...
    // 异步加载相关案例
    loadCases(record.id);
    if (window.AdsureTimeline && typeof window.AdsureTimeline.mount === "function") {
        window.AdsureTimeline.mount(record.id, detail);
    }
}
...
async function submitReview(notifyOperator = true) {
    ...
    const data = {
        ...
        review_intent: (currentRecord["审核状态"] === "已通过" || currentRecord["审核状态"] === "需修改") ? "edit" : "initial",
        expected_review_version: currentRecord.review_version,
    };
    ...
    try {
        ...
        if (result.operation_status === "succeeded") {
            ...
            loadRecords();
        } else if (result.operation_status === "stale") {
            await showLatestReview(currentRecord.id);
...
async function waitForReviewOperation(key, verdict, notifyOperator) {
    for (let attempt = 0; attempt < 40; attempt += 1) {
        ...
        try {
            ...
            if (result.operation_status === "stale") {
                await showLatestReview(currentRecord && currentRecord.id);
                return;
            }
...
async function showLatestReview(recordId) {
    reviewSubmissionKey = null;
    const message = "该物料已有最新审核结果，请刷新查看";
    try {
        const response = await fetch("/api/records");
        const records = await response.json();
        if (!response.ok || !Array.isArray(records)) throw new Error("records unavailable");
        allRecords = records;
        const latest = records.find(record => record.id === recordId);
        if (!latest) throw new Error("record unavailable");
        showDetail(latest);
        const reviewCard = document.querySelector(".review-card");
        if (reviewCard) {
            const notice = document.createElement("div");
            notice.setAttribute("role", "status");
            notice.style.cssText = "color:var(--danger);font-size:13px;margin-bottom:12px;";
            notice.textContent = message;
            reviewCard.insertBefore(notice, reviewCard.firstChild);
        }
    } catch (_error) {
...
async function loadRulesView() {
    ...
    const cards = corrections.slice().reverse().map(c => {
        ...
        const isRefine = c.feedback_type === "refine";
        const correctionLabel = isRefine ? "法务补充意见" : "正确判定";
        const correctionText = isRefine ? c.reason : c.correct_judgment;
        ...
        <div style="border:1px solid ${meta.border};border-radius:8px;margin-bottom:12px;overflow:hidden;${isPaused ? 'opacity:0.45;' : ''}">
            ...
            <div style="padding:12px 14px;display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                ...
                <div style="min-width:0;overflow-wrap:anywhere;">
                    <div style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">${correctionLabel}</div>
                    <div style="background:#f6ffed;border:1px solid #b7eb8f;border-radius:4px;padding:6px 10px;font-size:13px;color:#135200;font-weight:600;line-height:1.6;margin-bottom:10px;white-space:pre-wrap;">${escapeHtml(correctionText || "—")}</div>

                    ${!isRefine ? `
                    <div style="font-size:11px;font-weight:700;color:#888;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">驳回理由</div>
                    <div style="font-size:13px;color:#444;line-height:1.7;white-space:pre-wrap;">${escapeHtml(c.reason || "—")}</div>` : ""}
                </div>
            </div>
        </div>`;
    }).join("");

    const hasMemoryCenter = typeof window !== "undefined" && window.AdsureMemoryCenter;
    const memoryCenterMount = hasMemoryCenter ? '<div id="memory-center-mount"></div>' : "";
    const emptyState = corrections.length === 0 ? `
        ...
        <div style="padding:20px 24px;">
            <div class="rules-heading-row">
                <div>
                    ...
                    <div id="rules-summary" style="font-size:12px;color:var(--text-muted);">
                        共 ${corrections.length} 条纠正记录，其中 ${activeCount} 条已启用
                    </div>
                </div>
                ${memoryCenterMount}
            </div>
            <div id="rules-cards-wrapper">${emptyState}</div>
        </div>`;
    const memoryMount = document.getElementById("memory-center-mount");
    if (memoryMount && hasMemoryCenter) {
        try {
            window.AdsureMemoryCenter.initialize(memoryMount, {
                onStateChange: (enabled) => {
                    const summary = document.getElementById("rules-summary");
                    const wrapper = document.getElementById("rules-cards-wrapper");
                    if (summary) {
                        const currentActive = corrections.filter(c => c.status === "active").length;
                        summary.textContent = enabled
                            ? `共 ${corrections.length} 条纠正记录，其中 ${currentActive} 条已启用`
                            : `共 ${corrections.length} 条纠正记录，其中 ${currentActive} 条已启用（记忆中心已关闭，暂不参与审核）`;
                    }
                    if (wrapper) {
                        wrapper.style.opacity = enabled ? "" : "0.35";
