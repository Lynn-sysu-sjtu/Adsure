...
def markdown_escape(value: Any) -> str:
    """Public adapter for optional card modules using the same escaping rules."""
    return _md(value)
...
def _risk_color(risk: str) -> str:
    ...
    if risk.startswith("中"):
        return "orange"
    if risk.startswith("低") or risk == "无明显风险":
        return "green"
    return "blue"
    ...
    preview_elements = (
        ...
        if image_key
        else [
            {
                ...
                "text": {
                    "tag": "lark_md",
                    "content": f"**物料预览**\n{_md(preview or _no_attachment_hint)}",
                },
            }
        ]
    )
