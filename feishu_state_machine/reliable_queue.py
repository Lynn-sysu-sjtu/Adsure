...
class SQLiteQueue:
    ...
    def _ensure_schema(self):
        with self._schema_lock, self._connect() as conn:
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_audit_events_record_time
                ON audit_events(record_id, created_at, id);
                """
            )
            delivery_columns = {
                str(row["name"])
    ...
    def list_succeeded_legal_deliveries(self, record_id: str) -> list[dict]:
        with self._connect() as conn:
            rows = conn.execute(
                """SELECT * FROM deliveries
                   WHERE record_id = ? AND card_type = 'legal_review'
                     AND status = ? AND message_id IS NOT NULL
                   ORDER BY id""",
                (str(record_id), SUCCEEDED),
            ).fetchall()
        return [self._row(row) for row in rows]

    def list_delivery_summaries(
        self,
        *,
        page: int,
        page_size: int,
        ...
        if record_query:
            ...
            clauses.append("record_id LIKE ? ESCAPE '\\'")
        ...
        with self._connect() as conn:
            total = int(
                conn.execute(
                    f"SELECT COUNT(*) FROM deliveries {where}", tuple(params)
                ).fetchone()[0]
            )
            rows = conn.execute(
                f"""SELECT id, business_action, record_id, recipient_id,
                           card_type, message_id, status, attempts, max_attempts,
                           error_category, replay_count, created_at, updated_at
    ...
    def replay_terminal_delivery(self, delivery_id: int) -> dict:
        now = time.time()
        with self._connect() as conn:
            conn.execute("BEGIN IMMEDIATE")
            before = conn.execute(
                """SELECT id, record_id, status, message_id, replay_count
            ...
            cursor = conn.execute(
                """UPDATE deliveries
                   SET status = ?, attempts = 0, next_attempt_at = ?,
                       ...
                       error_category = NULL, replay_count = replay_count + 1,
                       updated_at = ?
                   WHERE id = ? AND status = ? AND message_id IS NULL""",
                (PENDING, now, now, int(delivery_id), TERMINAL_FAILED),
            )
            if cursor.rowcount == 1:
                after = conn.execute(
                    "SELECT record_id, replay_count FROM deliveries WHERE id = ?",
                    (int(delivery_id),),
                ).fetchone()
                conn.commit()
                return {
                    "outcome": "replayed",
                    "record_id": str(after["record_id"]),
                    "replay_count": int(after["replay_count"]),
                }
            conn.commit()
        if int(before["replay_count"] or 0) > 0:
        ...
        with self._connect() as conn:
            cursor = conn.execute(
                """INSERT OR IGNORE INTO audit_events (
                       record_id, event_type, event_key, actor_role, actor_name,
                       summary_json, job_id, delivery_id, created_at
                   ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    str(record_id),
                    str(event_type),
    ...
    def list_audit_events(self, record_id: str) -> list[dict]:
        with self._connect() as conn:
            rows = conn.execute(
