...
def _install_test_requests():
    ...
    requests.patch = lambda *_args, **_kwargs: _FakeResponse({"code": 0, "data": {}})
    ...
    def test_three_industry_fields_remain_available(self):
        ...
        for industry, industry_fields in samples.items():
            with self.subTest(industry=industry):
                ...
                self.assertRegex(normalized["review_version"], r"^[0-9a-f]{64}$")

    def test_stale_review_response_is_plain_and_nontechnical(self):
        payload, status = app_module._review_job_response({
            "status": "succeeded",
            "result": {"stale": True},
        })
        self.assertEqual(status, 409)
        self.assertEqual(payload["operation_status"], "stale")
        self.assertEqual(payload["message"], "该物料已有最新审核结果，请刷新查看")
        self.assertNotIn("version", json.dumps(payload, ensure_ascii=False))
