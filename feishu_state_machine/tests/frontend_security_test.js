...
const longRisk = "完整风险说明".repeat(100) + "；第二项风险\n最后一项风险";
const longReview = renderAiReviewCard({
    "预审_命中要点": longRisk,
    "预审_修改建议": longRisk,
});
assert(longReview.includes(longRisk), "long risk text must remain complete");
assert(longReview.includes('class="review-full-text"'), "risk text must preserve wrapping");
...
(async () => {
    ...
    vm.runInThisContext('currentRecord = {id: "rec-submit", review_version: "a".repeat(64), "审核状态": "待法务复核"}; reviewSubmissionKey = null;');
    ...
    assert(submittedBody.review_intent === "initial", "first decision must declare initial intent");
    assert(submittedBody.expected_review_version === "a".repeat(64), "review request must carry its opaque version");
    ...
    vm.runInThisContext('currentRecord = {id: "rec-submit", review_version: "b".repeat(64), "审核状态": "需修改"}; reviewSubmissionKey = null; sleep = async () => {};');
    let postCount = 0;
