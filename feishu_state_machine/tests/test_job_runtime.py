...
class JobRuntimeTests(unittest.TestCase):
    ...
    def test_legal_partial_delivery_retries_only_failed_recipient(self):
        ...
        self.assertTrue(all(row["payload"]["round"] == 1 for row in rows.values()))
        self.assertEqual(
    ...
    def test_audit_events_follow_action_and_ai_business_order(self):
        import audit_service

        with mock.patch("audit_service.audit_timeline_enabled", return_value=True):
            handle_card_action(
                CardActionRequest(
                    action="start_ai_review", record_id="rec-actions", round=1
                ),
                store=self.store,
            )
            action_types = [
                row["event_type"] for row in self.store.list_audit_events("rec-actions")
            ]
            self.assertEqual(action_types, ["ai_review_requested"])

            self.store.enqueue_job(
                "ai_review",
                "rec-ai",
                "ai-review-audit",
                {"mode": "深度", "round": 1, "operator_id": "ou-operator"},
            )
            record = {
                "fields": {
                    F_提交人: [{"id": "ou-operator", "name": "运营"}],
            ...
            with self._predictor(
                mock.Mock(return_value=("待法务复核", {"预审_风险等级": "高"}))
            ), mock.patch.object(
                feishu_api, "get_record", return_value=record
            ), mock.patch.object(
                feishu_api, "update_record"
            ), mock.patch.object(
                feishu_api, "get_dept_open_ids", return_value=["ou-legal"]
            ):
                while True:
                    job = self.store.claim_job("audit-direct")
                    if job and job["record_id"] == "rec-ai":
                        self.processor._run_job(job)
                        break
                    if job is None:
                        self.fail("AI audit job was not available")
                    self.store.complete_job(job["id"])

            event_types = [
                row["event_type"] for row in self.store.list_audit_events("rec-ai")
            ]
            self.assertEqual(
                event_types,
                [
    ...
    def test_audit_entry_failure_does_not_break_card_action(self):
        with mock.patch(
            "audit_service.record_event", side_effect=RuntimeError("SECRET_AUDIT")
        ):
            result = handle_card_action(
                CardActionRequest(
                    action="start_ai_review",
                    record_id="rec-audit-boundary",
                    event_id="evt-audit-boundary",
                    round=1,
                ),
                store=self.store,
            )
        ...
        self.assertEqual(self.store.get_job(result.job_id)["status"], "pending")

    def test_audit_entry_failure_does_not_break_business_job(self):
        queued = self.store.enqueue_job(
            "initial_submission", "rec-audit-job", "audit-boundary-job"
        )
        with mock.patch(
            "audit_service.record_event", side_effect=RuntimeError("SECRET_AUDIT")
        ), mock.patch.object(
    ...
    def test_audit_events_cover_submission_routes_resubmit_and_legal_completion(self):
        import audit_service
        ...
        def record_for(record_id):
            return {
                "fields": {
                    F_提交人: [{"id": "ou-operator", "name": "运营"}],
                    F_流转_当前状态: statuses[record_id],
                }
            }

        def run_job(job_type, record_id, key, payload=None):
            queued = self.store.enqueue_job(
                job_type, record_id, key, payload or {}, max_attempts=1
            )
            job = self.store.claim_job("audit-coverage")
            self.assertEqual(job["id"], queued.item_id)
            self.processor._run_job(job)
        ...
        legal_payload = {
            ...
            "notify_operator": False,
            "submitted_at_ms": 1700000000000,
            "request_hash": "audit-legal",
        }
        with mock.patch(
            "audit_service.audit_timeline_enabled", return_value=True
        ), mock.patch.object(
            feishu_api, "get_record", side_effect=record_for
        ), mock.patch.object(
            feishu_api, "update_record"
        ), mock.patch.object(
            feishu_api, "get_dept_open_ids", return_value=["ou-legal"]
            ...
            run_job("initial_submission", "rec-submit", "audit-submit")
            run_job(
                "route_to_legal",
                "rec-skip",
                "audit-skip",
            ...
            run_job(
                ...
                "rec-escalate",
                "audit-escalate",
                {"source": "escalate_to_legal", "round": 1},
            )
            run_job(
                "resubmit",
                "rec-resubmit",
                "audit-resubmit",
                {"round": 1, "operator_id": "ou-operator"},
            )
            run_job("legal_review", "rec-legal", "audit-legal", legal_payload)

        expected = {
            "rec-submit": ("material_submitted", None),
            "rec-skip": ("routed_to_legal", "skip_review"),
            "rec-escalate": ("routed_to_legal", "escalate_to_legal"),
            "rec-resubmit": ("material_resubmitted", None),
            "rec-legal": ("legal_review_completed", None),
        }
        for record_id, (event_type, source) in expected.items():
            events = self.store.list_audit_events(record_id)
            self.assertEqual([row["event_type"] for row in events], [event_type])
            if source:
                self.assertEqual(events[0]["summary"]["source"], source)
    ...
    def _run_full_submission(self, record_id, routing):
        ...
        with self._predictor(mock.Mock(return_value=(routing, {}))), mock.patch.object(
            ...
            # Step 2 – operator clicks "开启 AI 审核" → directly enqueues ai_review
            ...
            self.assertTrue(ctx.created, "start_ai_review should enqueue ai_review")
    ...
    def test_smoke_path_operator_modify(self):
        ...
        # 应依次投递：initial_review → operator_result
        self.assertEqual(
            card_types,
            ["initial_review", "operator_result"],
        )
        # 确认卡片内容：第一张是提交通知，第二张是修改意见
    ...
    def test_smoke_path_route_to_legal(self):
        ...
        # 应依次投递：initial_review → operator_transferred
