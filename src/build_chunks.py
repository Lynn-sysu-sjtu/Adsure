...
def base_metadata(case: dict, demo_production: bool = False) -> dict:
    ...
    return {
        ...
        "mapped_rule_ids": unique_list(case.get("mapped_rule_ids", [])),
        "penalty_amount": case.get("penalty_amount"),
        "review_status": case.get("review_status", ""),
        "raw_text_path": case.get("raw_text_path", ""),
        "owner_approval": (case.get("owner_approval") or {}) if owner_approval_override(case) else None,
...
def chunk_from_case(
    ...
    chunk = {
        ...
        "mapped_rule_ids": unique_list(case.get("mapped_rule_ids", [])),
        "search_fields": {
            "illegal_claims": unique_list(case.get("illegal_claims", [])),
            "risk_dimensions": unique_list(case.get("risk_dimensions", [])),
            "mapped_rule_ids": unique_list(case.get("mapped_rule_ids", [])),
            "product_or_service": case.get("product_or_service", ""),
            "keywords": unique_list(case.get("keywords", [])),
...
def owner_approval_override(case: dict) -> bool:
    """显式业主审批覆盖：仅当记录带完整的 owner_approval 审批块时，
    才放宽 source_type / source_url / source_verification_status 门槛；
    仍强制要求 approved_for_rag 与 review_status=approved。"""
    oa = case.get("owner_approval") or {}
    return bool(
        oa.get("approved") is True
        and oa.get("approved_for_rag") is True
        and oa.get("override_source_verification") is True
        and bool(oa.get("approved_by"))
        and bool(oa.get("approved_at"))
    )
...
def production_exclusion_reasons(case: dict) -> list[str]:
    ...
    source_type = case.get("source_type")
    if owner_approval_override(case):
        # 业主审批覆盖：跳过来源类型/URL/核验状态门槛（见 owner_approval 审批块），
        # 但仍需 approved_for_rag 与 review_status=approved（下方强制）。
        pass
    else:
