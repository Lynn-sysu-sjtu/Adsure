...
本仓库已经包含可运行的 FastAPI HTTP 服务、`POST /cases/retrieve` 和
`GET /health`，但尚未部署到双方可访问的长期运行环境。本节原有接口建议属于
历史设计，当前请求、响应和鉴权必须以《案例库接入接口确认稿》为准。

| 资产 | 路径 | 用途 | 是否可用于生产召回 |
| --- | --- | --- | --- |
| 已核验案例切片 | `data/chunks/production_chunks.json` | 正式案例 RAG | 是，当前为 10 个案例、20 个切片 |
...
截至 2026-07-19 的本地快照为：正式案例切片 20 条、通用候选切片 104 条、
三赛道候选切片 108 条、`/audit` 测试物料 20 条。切片是派生数据，联调时应
以重新执行构建命令后的实际计数为准。
...
如果测试入口是“※审心广告物料合规审核台 v4”而不是直接调用 `/audit`，不要把
下述英文接口字段直接当成 Base 列名。应使用
`data/audit_test_cases/base_v4_batch_create.json`；它按快照中的 58 个真实字段
生成，只写可写的 ①运营/行业输入字段及初始流转状态。字段映射见
`docs/Base v4测试样例字段映射.md`。
...
- [x] 已确定采用独立 HTTP 案例检索服务：`POST /cases/retrieve`；
- [ ] 飞书字段 ID 与接口字段映射；
- [ ] AI 模型接入方式及测试环境可用性；
