...
def hotword_prompt(industry: str) -> str:
    """Build a neutral product-term priming prompt for Whisper.

    Only product/industry nouns are allowed; a regression guard rejects
    legal judgement terms so priming cannot manufacture findings.
    """
    try:
    ...
    words = industries.get(industry) or industries.get("一般行业") or []
    words = words[: int(payload.get("max_words_per_industry", 64))]
    if not words:
        return ""
    lead = (f"以下是一段普通话{industry}广告口播，可能包含以下词汇："
            if industry in industries and industry != "一般行业"
            else "以下是一段普通话广告口播，可能包含以下词汇：")
...
def readiness() -> dict:
    ...
    runtime = load_env({"VIDEO_MVP_ASR_ENGINE"})
    status = {"provider": "faster-whisper", "model": str(path), "chinese": chinese_readiness(),
            "engine_policy": runtime.get("VIDEO_MVP_ASR_ENGINE", "dual"),
    ...
    try:
        from .volc_asr import volc_readiness
        status["volc"] = volc_readiness()
    except Exception as exc:  # config file issues must not break the health endpoint
        status["volc"] = {"configured": False, "error": type(exc).__name__}
    return status
...
def transcribe_video(video: Path, output: Path, industry: str = "一般行业") -> tuple[list[EvidenceUnit], dict]:
    """Isolate native inference so a hung decoder/model cannot hang a job forever."""
    raw_path = output / "asr_raw.json"
    command = [sys.executable, "-m", "src.video_mvp.asr", "--video", str(video),
               "--output", str(raw_path), "--industry", industry or "一般行业"]
    try:
...
def run_worker(video: Path, industry: str = "一般行业") -> dict:
    from .chinese_asr import readiness as chinese_readiness, run_worker as chinese_worker, compare_transcripts
    engine = load_env({"VIDEO_MVP_ASR_ENGINE"}).get("VIDEO_MVP_ASR_ENGINE", "dual")
    if engine not in {"dual", "sensevoice", "whisper", "volc"}:
        raise ValueError("VIDEO_MVP_ASR_ENGINE must be dual, sensevoice, whisper, or volc")
    if engine == "volc":
        from .volc_asr import transcribe_worker as volc_worker, volc_readiness
        volc_status = volc_readiness()
        if not volc_status["configured"]:
            raise RuntimeError("火山语音引擎未配置凭据，无法使用 VIDEO_MVP_ASR_ENGINE=volc")
        primary = volc_worker(video, industry)
        # 云端主识别仍保留本地 SenseVoice 独立假设，用于产生分歧复核信号。
        ready = chinese_readiness()
        if ready["dependency_ready"] and ready["model_ready"]:
            try:
                ...
                primary["quality_status"] = ("engines_disagree" if primary["engine_disagreements"]
                                             else "cross_checked_unverified")
            ...
            primary["fallback_reason"] = "本地中文校验模型未就绪，仅完成云端单引擎识别，未做双引擎核验"
        return primary
    ready = chinese_readiness()
    ...
    primary = _whisper_worker(video, industry)
    if engine == "dual":
        try:
            verification = chinese_worker(video)
            primary["verification"] = verification
...
def _whisper_worker(video: Path, industry: str = "一般行业") -> dict:
    import av
    import numpy as np
    ...
    payload = {"status": "no_audio_track", "provider": "faster-whisper", "model": str(model_path()),
               "model_provenance": model_provenance(), "hotword_industry": industry,
        ...
        for info in track_info:
            ...
            try:
                ...
                transcribe_kwargs = dict(
                    language="zh", beam_size=5, word_timestamps=True,
                    vad_filter=True, vad_parameters={"min_silence_duration_ms": 400},
                    condition_on_previous_text=False,
                )
                prompt = hotword_prompt(industry)
                if prompt:
                    transcribe_kwargs["initial_prompt"] = prompt
                segments, metadata = model.transcribe(audio, **transcribe_kwargs)
                count = 0
...
def main() -> None:
    ...
    parser.add_argument("--industry", default="一般行业")
    ...
    if args.download_model:
        ...
        args.output.write_text(json.dumps(run_worker(args.video, args.industry), ensure_ascii=False, indent=2), encoding="utf-8")
    else:
