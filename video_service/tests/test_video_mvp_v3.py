...
class HotwordGuardTests(unittest.TestCase):
    FORBIDDEN = ("国家级","最高级","最佳","第一","唯一","顶级","极品","根治","治愈",
                 "治疗","预防疾病","保证","100%","副作用","零风险","稳赚","包过",
                 "无效退款","特效","万能","神器")

    def test_hotwords_are_neutral_terms_only(self):
        data = json.loads((ROOT / "data/rules/asr_hotwords.json").read_text(encoding="utf-8"))
        self.assertLessEqual(len(data["industries"]["化妆品"]), data["max_words_per_industry"])
        for industry, words in data["industries"].items():
            self.assertTrue(all(isinstance(w, str) and w for w in words), industry)
            self.assertEqual(len(words), len(set(words)), f"{industry} 热词重复")
            for word in words:
                for banned in self.FORBIDDEN:
                    self.assertNotIn(banned, word, f"热词不得含法律评价词：{industry}/{word}")

    def test_hotword_prompt_by_industry_and_fallback(self):
        from src.video_mvp.asr import hotword_prompt
        hotword_prompt.cache_clear()
        self.assertIn("粉底液", hotword_prompt("化妆品"))
        self.assertIn("化妆品广告口播", hotword_prompt("化妆品"))
        self.assertIn("直播间", hotword_prompt("不存在的行业"))
        hotword_prompt.cache_clear()
        with patch("src.video_mvp.asr.HOTWORD_PATH", Path("/tmp/adsure-no-hotwords.json")):
            self.assertEqual("", hotword_prompt("化妆品"))
        hotword_prompt.cache_clear()

    def test_cosmetic_exaggeration_scoped_to_cosmetics(self):
        from src.video_mvp.rules import analyze_rules
        def found(industry):
            risks, _ = analyze_rules(
                [EvidenceUnit(id="a", source="asr", kind="text",
                              text="这款粉底液堪称瑕疵橡皮擦", t_start=0, t_end=2)],
                industry=industry, frame_count=2, coverage_complete=True, sample_interval=.5)
            return risks
        hit = found("化妆品")
        self.assertEqual(["cosmetic_exaggeration"], [r.pattern_id for r in hit])
        self.assertIn("ADLAW-028", hit[0].rule_ids)
        self.assertEqual([], [r for r in found("一般行业") if r.pattern_id == "cosmetic_exaggeration"])
