...
class CardActionServiceTests(unittest.TestCase):
    ...
    def test_http_and_websocket_same_event_create_one_job(self):
        common = dict(
            action="start_ai_review", record_id="rec-1", operator_id="ou-1",
            event_id="event-shared", round=1,
        )
        http = handle_card_action(CardActionRequest(**common, transport="http"), store=self.store)
        websocket = handle_card_action(
            CardActionRequest(**common, transport="websocket"), store=self.store,
        )
        ...
        for action in ("skip_review", "escalate_to_legal"):
            with self.subTest(action=action):
                result = handle_card_action(CardActionRequest(
                    action=action, record_id=f"rec-{action}", event_id=f"evt-{action}", round=1,
                ), store=self.store)
    ...
    def test_valid_legacy_confirm_mode_is_supported(self):
        with mock.patch(
            "card_action_service._legacy_record_status", return_value="运营起草",
        ) as state_read:
            result = handle_card_action(CardActionRequest(
                action="confirm_mode", mode="深度", record_id="rec-1", event_id="evt-mode",
            ), store=self.store)
    ...
    def test_unknown_or_invalid_legacy_action_is_stale_without_details(self):
        unknown = handle_card_action(CardActionRequest(
            action="unknown", record_id="rec-1", event_id="evt-unknown",
        ), store=self.store)
        invalid_mode = handle_card_action(CardActionRequest(
            action="confirm_mode", mode="内部模式", record_id="rec-1", event_id="evt-mode",
        ), store=self.store)
    ...
    def test_resubmit_duplicate_clicks_share_one_job_and_next_round_is_allowed(self):
        first = handle_card_action(CardActionRequest(
            action="resubmit", record_id="rec-1", event_id="evt-1", round=1,
        ), store=self.store)
        duplicate = handle_card_action(CardActionRequest(
            action="resubmit", record_id="rec-1", event_id="evt-1", round=1,
        ), store=self.store)
        second = handle_card_action(CardActionRequest(
            action="resubmit", record_id="rec-1", event_id="evt-2", round=1,
        ), store=self.store)
        ...
        next_round = handle_card_action(CardActionRequest(
            action="resubmit", record_id="rec-1", event_id="evt-3", round=2,
        ), store=self.store)
    ...
    def test_start_then_skip_creates_only_one_initial_choice(self):
        start = handle_card_action(CardActionRequest(
            action="start_ai_review", record_id="rec-start-first", event_id="evt-start", round=1,
        ), store=self.store)
        skip = handle_card_action(CardActionRequest(
            action="skip_review", record_id="rec-start-first", event_id="evt-skip", round=1,
        ), store=self.store)
        ...
        self.assertEqual(jobs[0]["job_type"], "ai_review")
        self.assertEqual(jobs[0]["idempotency_key"], "initial-choice:rec-start-first:round:1")

    def test_skip_then_start_creates_only_one_initial_choice(self):
        skip = handle_card_action(CardActionRequest(
            action="skip_review", record_id="rec-skip-first", event_id="evt-skip", round=1,
        ), store=self.store)
        start = handle_card_action(CardActionRequest(
            action="start_ai_review", record_id="rec-skip-first", event_id="evt-start", round=1,
        ), store=self.store)
        ...
        self.assertEqual(jobs[0]["idempotency_key"], "initial-choice:rec-skip-first:round:1")

    def test_unversioned_action_with_wrong_source_status_is_stale(self):
        with mock.patch(
            "card_action_service._legacy_record_status", return_value="待法务复核",
        ) as state_read:
            result = handle_card_action(CardActionRequest(
                action="skip_review", record_id="rec-old", event_id="evt-old",
            ), store=self.store)
    ...
    def test_escalate_key_does_not_block_later_resubmit(self):
        escalated = handle_card_action(CardActionRequest(
            action="escalate_to_legal", record_id="rec-followup",
            event_id="evt-escalate", round=1,
        ), store=self.store)
        resubmitted = handle_card_action(CardActionRequest(
            action="resubmit", record_id="rec-followup",
            event_id="evt-resubmit", round=1,
        ), store=self.store)
